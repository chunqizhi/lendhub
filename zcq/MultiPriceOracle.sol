...
// File: contracts/Governance/MultiPriceOracle.sol

pragma solidity ^0.5.16;



contract MultiPriceOracle is PriceOracle {
    ...
    function addRole(address addr) public {
        require(msg.sender == admin, "only the admin may call addRole");
        members[addr] = true;
    }

    function dropRole(address addr) public {
        require(msg.sender == admin, "only the admin may call dropRole");
        members[addr] = false;
    }
    ...
    function setUnderlyingPrices(CToken[] memory cTokens, uint[] memory underlyingPrices) public {
        require(cTokens.length == underlyingPrices.length, "underlying prices number error");
        uint len = cTokens.length;
        for (uint i = 0; i < len; i++) {
            setUnderlyingPrice(cTokens[i], underlyingPrices[i]);
        }
    }
    function setUnderlyingPrice(CToken cToken, uint underlyingPriceMantissa) public {
        require(hasRole(msg.sender), "only the committer may call setUnderlyingPrice");
    ...
    function setDirectPrices(address[] memory assets, uint[] memory prices) public {
        require(assets.length == prices.length, "assets prices number error");
        uint len = assets.length;
        for (uint i = 0; i < len; i++) {
            setDirectPrice(assets[i], prices[i]);
        }
    }
